"""
Async SEA (Statement Execution API) backend implementation.

This module provides an async implementation of the DatabricksClient interface
using the Statement Execution API with true async/await support.
"""
...
from databricks.sql.backend.sea.utils.async_http_client import AsyncSeaHttpClient
...
class AsyncSeaDatabricksClient(AsyncDatabricksClient):
    """
    Async Statement Execution API (SEA) implementation of the AsyncDatabricksClient interface.

    This class provides true async/await support for all SEA operations using aiohttp
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize the async SEA backend client.
        ...
        """
        logger.debug(
            "AsyncSeaDatabricksClient.__init__(server_hostname=%s, port=%s, http_path=%s)",
            server_hostname,
            port,
            http_path,
        )
        ...
        self._http_client = AsyncSeaHttpClient(
    ...
    async def open_session(
        ...
        """Opens a new session with the Databricks SQL service using SEA asynchronously."""
        logger.debug(
            "AsyncSeaDatabricksClient.open_session(session_configuration=%s, catalog=%s, schema=%s)",
        ...
        response = await self._http_client._make_request(
    ...
    async def close_session(self, session_id: SessionId) -> None:
        """Closes an existing session with the Databricks SQL service asynchronously."""
        logger.debug("AsyncSeaDatabricksClient.close_session(session_id=%s)", session_id)
        ...
        await self._http_client._make_request(
            method="DELETE",
    ...
    def _extract_description_from_manifest(
        ...
        for col_data in columns_data:
            ...
            # Normalize SEA type to Thrift conventions
            type_name = normalize_sea_type_to_thrift(type_name, col_data)

            # Strip _TYPE suffix and convert to lowercase
            type_name = (
    ...
    def _check_command_not_in_failed_or_closed_state(
        ...
        """Check if command is in failed or closed state and raise appropriate exception."""
        state = status.state
    ...
    async def _wait_until_command_done(
        ...
        while final_response.status.state in [
            ...
            final_response = await self._poll_query(command_id)
    ...
    async def execute_command(
    ...
    ) -> Union["AsyncSeaResultSet", None]:
        """Execute a SQL command using the SEA backend asynchronously."""
        # Import here to avoid circular imports
        from databricks.sql.backend.sea.async_result_set import AsyncSeaResultSet
        ...
        response_data = await self._http_client._make_request(
        ...
        if response.status.state != CommandState.SUCCEEDED:
            final_response = await self._wait_until_command_done(response)

        return await self._response_to_result_set(final_response, cursor)

    async def _response_to_result_set(
        self,
        response: Union[ExecuteStatementResponse, GetStatementResponse],
        cursor: "AsyncCursor",
    ) -> "AsyncSeaResultSet":
        """Convert a SEA response to an AsyncSeaResultSet."""
        from databricks.sql.backend.sea.async_result_set import AsyncSeaResultSet

        execute_response = self._results_message_to_execute_response(response)

        return AsyncSeaResultSet(
    ...
    async def close_command(self, command_id: CommandId) -> None:
        """Close a command and release resources asynchronously."""
        if command_id.backend_type != BackendType.SEA:
            raise ValueError("Not a valid SEA command ID")
    ...
    async def _poll_query(self, command_id: CommandId) -> GetStatementResponse:
        """Poll for the current command info asynchronously."""
        if command_id.backend_type != BackendType.SEA:
            raise ValueError("Not a valid SEA command ID")
    ...
    async def get_query_state(self, command_id: CommandId) -> CommandState:
        """Get the state of a running query asynchronously."""
        response = await self._poll_query(command_id)
    ...
    async def get_execution_result(
        ...
        """Get the result of a command execution asynchronously."""
        response = await self._poll_query(command_id)
        return await self._response_to_result_set(response, cursor)

    async def get_chunk_links(
        self, statement_id: str, chunk_index: int
    ) -> List[ExternalLink]:
        """Get links for chunks starting from the specified index asynchronously."""
        response_data = await self._http_client._make_request(
            method="GET",
    ...
    async def get_catalogs(
        ...
        """Get available catalogs by executing 'SHOW CATALOGS' asynchronously."""
    ...
    async def get_schemas(
        ...
        """Get schemas by executing 'SHOW SCHEMAS IN catalog [LIKE pattern]' asynchronously."""
        if not catalog_name:
